...
//    }
  ...
  spamtimer(SpamDelay) {// non blocking delay before printing again. This skips the following code when delay time (ms) hasn't been met
   ...
   if(digitalRead(14) == HIGH){activeMPU = 0; // Serial.println("Up14");
    ...
    if(digitalRead(12) == HIGH) {activeMPU = 1; // Serial.println("Up12");
   ...
   /* if(data.length() > 50){
     ...
     Serial.println("PRint");
      ...
      Serial.println("Error: Failed to transfer all of the information") ;
    }
     data = "";
    }*/
 //  client.stop();
...
void setup() {
  ...
  mpu.SetAddress(MPU6050_DEFAULT_ADDRESS);
    mpu.Set_DMP_Output_Rate_Hz(15);           // Set the DMP output rate from 200Hz to 5 Minutes.
...
void loop() {
 ...
 // mpu.dmp_read_fifo(0);// Must be in loop
  delay(500); // this delay is very important. Otherwise there my be skippes
